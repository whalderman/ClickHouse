...
MergeTreeIndexPtr ginIndexCreator(const IndexDescription & index)
{
    ...
    auto token_extractor{
        [&tokenizer, &options] -> std::unique_ptr<ITokenExtractor>
        {
            ...
            else if (tokenizer == GIN_TOKENIZER_CHINESE)
                return std::make_unique<ChineseTokenExtractor>();
            else if (tokenizer == GIN_TOKENIZER_NGRAM)
            {
                const auto ngram_n{extract_uint64_option(options, "tokenizer_ngram_options")};
                return std::make_unique<NgramTokenExtractor>(ngram_n.value());
            }
...
void ginIndexValidator(const IndexDescription & index, bool /*attach*/)
{
    ...
    if (arguments.empty())
        throw Exception(ErrorCodes::INCORRECT_QUERY, "GIN index must have at least 'tokenizer' argument");
    ...
    if (tokenizer.value() == GIN_TOKENIZER_NGRAM)
    {
        const auto ngram_n{extract_uint64_option(options, "tokenizer_ngram_options")};
        if (!ngram_n.has_value())
            throw Exception(ErrorCodes::INCORRECT_QUERY, "GIN index must include 'tokenizer_ngram_options'");

        if (ngram_n.value() < 2 || ngram_n.value() > 8)
            throw Exception(
                ErrorCodes::INCORRECT_QUERY,
                "GIN index 'tokenizer_ngram_options' argument must be between 2 and 8, but got {}",
                ngram_n.value());
    }
